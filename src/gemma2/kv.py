"""
gemma2/kv.py — the family KV cache (lightweight).
...
Note: Gemma2's local (sliding-window) layers still cache the FULL (K, V) — the window is
applied in the attention *mask*, not by truncating the cache — so every slot is a plain KV
tuple, the same as the global layers.
"""
...
class Gemma2Cache:
    ...
    def clone(self):
        new = Gemma2Cache(len(self.layers))
